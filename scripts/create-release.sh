...
GITHUB_REPO="hanruihua/agentic-island"

# Website repo for auto-updating appcast
WEBSITE_DIR="${AGENTIC_ISLAND_WEBSITE:-$PROJECT_DIR/../AgenticIsland-website}"
WEBSITE_PUBLIC="$WEBSITE_DIR/public"

APP_PATH="$EXPORT_PATH/Agentic Island.app"
APP_NAME="AgenticIsland"
KEYCHAIN_PROFILE="AgenticIsland"
...
if command -v create-dmg &> /dev/null; then
    ...
    create-dmg \
        --volname "Agentic Island" \
        --window-size 600 400 \
        --icon-size 100 \
        --icon "Agentic Island.app" 150 200 \
        --app-drop-link 450 200 \
        --hide-extension "Agentic Island.app" \
        "$DMG_PATH" \
        "$APP_PATH"
else
    echo "Using hdiutil (install create-dmg for prettier DMG: brew install create-dmg)"
    hdiutil create -volname "Agentic Island" \
        -srcfolder "$APP_PATH" \
        -ov -format UDZO \
        "$DMG_PATH"
fi
...
POSSIBLE_PATHS=(
    "$HOME/Library/Developer/Xcode/DerivedData/AgenticIsland-*/SourcePackages/artifacts/sparkle/Sparkle/bin"
)
...
if ! command -v gh &> /dev/null; then
    ...
    if gh release view "v$VERSION" --repo "$GITHUB_REPO" &>/dev/null; then
        ...
        gh release create "v$VERSION" "$DMG_PATH" \
            --repo "$GITHUB_REPO" \
            --title "Agentic Island v$VERSION" \
            --notes "## Agentic Island v$VERSION

### Installation
1. Download \`$APP_NAME-$VERSION.dmg\`
2. Open the DMG and drag Agentic Island to Applications
3. Launch Agentic Island from Applications

### Auto-updates
After installation, Agentic Island will automatically check for updates."
    fi

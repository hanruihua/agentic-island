...
struct ChatView: View {
    ...
    private var chatHeader: some View {
        Button {
            ...
            HStack(spacing: 8) {
                Image(systemName: "chevron.left")
                    ...
                    .foregroundColor(isHeaderHovered ? TerminalColors.ivory : TerminalColors.warmSilver)
                    .frame(width: 24, height: 24)

                Text(session.displayTitle)
                    .font(.system(size: 14, weight: .semibold))
                    .foregroundColor(isHeaderHovered ? TerminalColors.ivory : TerminalColors.ivory.opacity(0.85))
                    .lineLimit(1)
            ...
            .background(
                RoundedRectangle(cornerRadius: 8)
                    .fill(isHeaderHovered ? TerminalColors.backgroundHover : Color.clear)
            )
        }
        .buttonStyle(.plain)
        ...
        .padding(.vertical, 4)
        .background(TerminalColors.nearBlack.opacity(0.6))
    ...
    private var loadingState: some View {
        VStack(spacing: 8) {
            ProgressView()
                .progressViewStyle(CircularProgressViewStyle(tint: TerminalColors.stoneGray))
                .scaleEffect(0.8)
    ...
    private var emptyState: some View {
        VStack(spacing: 8) {
            Image(systemName: "bubble.left.and.bubble.right")
                .font(.system(size: 24))
                .foregroundColor(TerminalColors.stoneGray.opacity(0.5))
    ...
    private let fadeColor = Color(hex: 0x141413) // Warm near-black fade
    ...
    /// Can send messages if session has a TTY (tmux or direct)
    ...
    private var inputBar: some View {
        HStack(spacing: 10) {
            TextField(canSendMessages ? "Message Claude..." : "No TTY available for messaging", text: $inputText)
                .textFieldStyle(.plain)
                .font(.system(size: 13))
                .foregroundColor(canSendMessages ? TerminalColors.ivory : TerminalColors.stoneGray)
                ...
                .background(
                    RoundedRectangle(cornerRadius: 20)
                        .fill(canSendMessages ? TerminalColors.surface.opacity(0.5) : TerminalColors.surface.opacity(0.25))
                        .overlay(
                            RoundedRectangle(cornerRadius: 20)
                                .strokeBorder(TerminalColors.borderDark, lineWidth: 1)
                        )
                )
                .onSubmit {
                    sendMessage()
            ...
            Button {
                ...
                Image(systemName: "arrow.up.circle.fill")
                    .font(.system(size: 28))
                    .foregroundColor(!canSendMessages || inputText.isEmpty ? TerminalColors.stoneGray : TerminalColors.prompt)
            }
            .buttonStyle(.plain)
    ...
    private func sendToSession(_ text: String) async {
        ...
        if session.isInTmux, let target = await findTmuxTarget(tty: tty) {
            ...
            // Direct TTY write for non-tmux sessions
            await sendViaTTY(text: text, tty: tty)
        }
    }

    private func sendViaTTY(text: String, tty: String) async {
...
struct UserMessageView: View {
    ...
    var body: some View {
        HStack {
            ...
            MarkdownText(text, color: TerminalColors.ivory, fontSize: 13)
                .padding(.horizontal, 14)
                .padding(.vertical, 10)
                .background(
                    RoundedRectangle(cornerRadius: 18)
                        .fill(TerminalColors.prompt.opacity(0.2))
                )
        }
    }
}
...
struct AssistantMessageView: View {
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 6) {
            // Warm dot indicator
            Circle()
                .fill(TerminalColors.warmSilver)
                .frame(width: 6, height: 6)
                .padding(.top, 5)

            MarkdownText(text, color: TerminalColors.ivory.opacity(0.9), fontSize: 13)
...
struct ProcessingIndicatorView: View {
    ...
    private let color = TerminalColors.prompt // Terracotta brand
...
struct ToolCallView: View {
    ...
    private var textColor: Color {
        switch tool.status {
        case .running:
            return TerminalColors.warmSilver
        case .waitingForApproval:
            return TerminalColors.coral
        case .success:
            return TerminalColors.warmSilver
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 6)
                .fill(canExpand && isHovering ? TerminalColors.surface.opacity(0.3) : Color.clear)
        )
...
struct SubagentToolRow: View {
    ...
    private var statusColor: Color {
        switch tool.status {
        case .running, .waitingForApproval: return TerminalColors.coral
        case .success: return TerminalColors.green
        case .error, .interrupted: return TerminalColors.red
        }
    }
...
struct SubagentToolsSummary: View {
    ...
    var body: some View {
                ...
                ForEach(toolCounts.prefix(5), id: \.0) { name, count in
                    HStack(spacing: 2) {
                        ...
                        Text("×\(count)")
                            ...
                            .foregroundColor(TerminalColors.stoneGray.opacity(0.7))
                    }
                }
            }
        }
        ...
        .background(
            RoundedRectangle(cornerRadius: 6)
                .fill(TerminalColors.surface.opacity(0.2))
        )
    }
}
    ...
    var body: some View {
        HStack(spacing: 12) {
            ...
            Button {
                ...
                .foregroundColor(isInTmux ? TerminalColors.ivory : TerminalColors.stoneGray)
                .padding(.horizontal, 16)
                .padding(.vertical, 8)
                .background(isInTmux ? TerminalColors.prompt : TerminalColors.surface)
                .clipShape(Capsule())
            }
            .buttonStyle(.plain)
...
struct NewMessagesIndicator: View {
    ...
    var body: some View {
        Button(action: onTap) {
            ...
            .background(
                Capsule()
                    .fill(TerminalColors.prompt) // Terracotta brand
